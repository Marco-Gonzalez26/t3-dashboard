...
function Edit() {
  ...
  const onSubmit = handleSubmit(async (mutateData) => {
    if (mutateData != null && data) {
      const mutateFirstAppointment = new Date(mutateData.primeraCita!);
      const dataFirstAppointment = new Date(data.primeraCita!);

      const mutateControl = new Date(mutateData.control!);
      const dataControl = new Date(data.control!);
      console.log({ dataControl, mutateControl });
      await updatePatientMutation.mutate(
        {
          id: query?.id,
          nombre: mutateData?.nombre || data?.nombre,
          edad: Number(mutateData?.edad) || Number(data?.edad),
          nacimiento: mutateData?.nacimiento || data?.nacimiento,
          talla: mutateData?.talla || data?.talla,
          peso: Number(mutateData?.peso) || Number(data?.peso),
          ...
          control: mutateControl || dataControl,
          primeraCita: mutateFirstAppointment || dataFirstAppointment,
                    ...
                    <div className="grid grid-cols-6 gap-6">
                      ...
                      <div className="col-span-6 sm:col-span-4 lg:col-span-2">
                        ...
                        <input
                          defaultValue={
                            data?.primeraCita
                              ? data?.primeraCita.toISOString().substring(0, 10)
                              : "No hay información"
                          }
                          type="date"
                      ...
                      <div className="col-span-6 sm:col-span-3 lg:col-span-2">
                        ...
                        <input
                          defaultValue={
                            data?.control
                              ? data?.control.toISOString().substring(0, 10)
                              : ""
                          }
                          type="date"
                          {...register("control")}
